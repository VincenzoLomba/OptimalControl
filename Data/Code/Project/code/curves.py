...
class CurveType(Enum): sigmoid = "sigmoid"; cubic = "cubic"; esponential = "esponential"
...
def generateCurves(xxValues, uuValues, ttValues, curveTypeValues):
    ...
    Each segment of the curve can be generated by using a sigmoid, a cubic spline or an esponential spline.
    Arguments:
    ...
    for ii in range(numberOfPoints-1):
        # Generating the esponential spline trajectory bewtween the two points given by index ii and ii+1
        ...
        for iis in range(ns):
            ...
            elif splineType == CurveType.esponential:
                xx[iis, firstTimeIstant:secondTimeIstant] = esponentialSpline(ttValues[ii], ttValues[ii+1], xxValues[iis, ii], xxValues[iis, ii+1], dt)[0]
        ...
        for iii in range(ni):
            ...
            if (uuValues[iii, ii] == uuValues[iii, ii+1]):
                ...
                uu[iii, firstTimeIstant:secondTimeIstant] = esponentialSpline(ttValues[ii], ttValues[ii+1], uuValues[iii, ii], uuValues[iii, ii+1], dt)[0]
            elif splineType == CurveType.cubic:
...
def esponentialSpline(t1, t2, v1, v2, dt):
    """
    ...
    def esponentialSplineFunctions(k, A, B, C, D):
    ...
    def curvatureVariance(k):
        if k <= 0: return inf
        A, B, C, D = solveEsponentialSpline(k)
        _, dfdt, d2fd2t = esponentialSplineFunctions(k, A, B, C, D)
    ...
    A, B, C, D = solveEsponentialSpline(kk)
    f, _, _ = esponentialSplineFunctions(kk, A, B, C, D)
