...
def dynamics(xx,uu):
    """
    Discretized Dynamics of an Flexible Robotic Arm
    Arguments:
    - xx: 4x1 column vector state at time t x=[x0, x1, x2, x3]'=[ϑ1, ϑ2, dϑ1dt, dϑ2dt]'
    - uu: 1x1 input at time 7
    Returns:
    ...
    """
    xx = xx.squeeze() # remove eventually present singleton dimensions
    ...
    xxp[0] = xx[0] + dt*xx[2]
    xxp[1] = xx[1] + dt*xx[3]
    xxp[2:4] = xx[2:4] + dt*invM@(U-G-F@array([xx[2], xx[3]])-C)
    ...
    dfdx[2:4, 2] = array([1, 0])+dt*(-dGdx[2]-F*array([1, 0])-dCdx[2]);
    dfdx[2:4, 3] = array([0, 1])+dt*(-dGdx[3]-F*array([0, 1])-dCdx[3]);
