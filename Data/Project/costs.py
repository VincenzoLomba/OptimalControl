...
def stageCostTrkTrj(xx, uu, xx_des, uu_des, QQt, RRt):
    """
    ...
    Arguments:
    - xx: nsx1 actual column vector state at time t (recall: x=[x0, x1, x2, x3]'=[ϑ1, ϑ2, dϑ1dt, dϑ2dt]' in the FRA case)(from a python variable p.o.f., this is of shape (ns,))
    - xx_des: nsx1 desired column vector state at time t (from a python variable p.o.f., this is of shape (ns,))
    - uu: nix1 actual input value at time t (from a python variable p.o.f., this is of shape (ni,))
    - uu_des: nix1 desired input value at time t (from a python variable p.o.f., this is of shape (ni,))
    - QQt: nsxns state cost matrix at time t
    - RRt: nixni input cost matrix at time t
    Returns:
    - ll: scalar stage cost at xx,uu
    - dldx: nsx1 gradient of the cost wrt x at xx,uu (recall: the gradient is the transpose of the Jacobian and viceversa)(from a python variable p.o.f., this is of shape (ns,))
    - dldu: nix1 gradient of the cost wrt u at xx,uu (recall: the gradient is the transpose of the Jacobian and viceversa)(from a python variable p.o.f., this is of shape (ni,))
...
def termCostTrkTrj(xT, xT_des, QQT):
    ...
    Arguments:
    - xT: nsx1 actual column vector state at the terminal time T (from a python variable p.o.f., this is of shape (ns,))
    - xT_des: nsx1 desired column vector state at the terminal time T (from a python variable p.o.f., this is of shape (ns,))
    - QQT: nsxns terminal state cost matrix
    Returns:
    - ll: scalar terminal cost at xT
    - dldx: nsx1 gradient of the cost wrt x at xT (from a python variable p.o.f., this is of shape (ns,))
